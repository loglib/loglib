...
app.get("/vitals", async (c) => {
  ...
  try {
    const res = await eventDB
      .getVital(startDateObj, endDateObj, websiteId)
      .then((res) =>
        res.map((r) => {
    ...
    console.log(res);
    const vitalsByDate = getVitalsByDate(
      res,
      startDateObj,
      endDateObj,
      timezone
    ...
    for (const vital of res) {
      switch (vital.name) {
        case "CLS":
          cls.push(vital.delta);
          break;
        case "LCP":
          lcp.push(vital.delta);
          break;
        case "FID":
          fid.push(vital.delta);
          break;
        case "INP":
          inp.push(vital.delta);
          break;
        case "FCP":
          fcp.push(vital.delta);
          break;
      }
    }
    ...
    return c.json(
      {
        cls: (CLS / cls.length) * 1000,
        lcp: LCP / lcp.length / 10000,
        fid: FID / fid.length,
        inp: inp.sort((a, b) => b - a)[0],
        fcp: FCP / fcp.length / 10000,
        vitalsByDate: vitalsByDate.dataByDate,
        data: vitalsByDate.data,
      },
      200
    );
  } catch (e) {
