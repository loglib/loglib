...
export const stats = [{
    name: "Largest Contentful Paint", short: "LCP", description: (
        <p>
    ...
    ), formatter: (data: number) => `${(data / 1000).toFixed(2)} sec`, getRating: (data: number) => {
        const [good, improve] = LCPThresholds
        return ({ style: getStyle(data, good, improve), label: `${data <= good ? "Good" : data <= improve ? "Needs Improvement" : "Poor"}` })
    }
}, {
    name: "First Contentful Paint", short: "FCP", description: (
        <p>
    ...
    name: "Interaction Paint", short: "INP", description: (
        <p>
    ...
    ), formatter: (data: number) => `${data.toFixed(2)} ms`, getRating: (data: number) => {
        const [good, improve] = INPThresholds
        return ({ style: getStyle(data, good, improve), label: `${data <= good ? "Good" : data <= improve ? "Needs Improvement" : "Poor"}` })
    }
}, {
    name: "Content Layout Shift", short: "CLS", description: (
        <p>
    ...
    ), formatter: (data: number) => `${(data).toFixed(2)}`, getRating: (data: number) => {
        const [good, improve] = CLSThresholds
        return ({ style: getStyle(data, good, improve), label: `${data <= good ? "Good" : data <= improve ? "Needs Improvement" : "Poor"}` })
    }
}, {
    name: "First Interaction Delay", short: "FID", description: (
        <p>
