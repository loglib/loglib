        ...
        <Button onClick={() => {
            if (disabled) {
                toast({
                    ...
                    description: "You need to add a website first before you can generate an api key.",
                    variant: "destructive"
                })
            } else {
                setModal(true)
            }
        }} variant="secondary" className=" llc-ctc">
            Generate New Key
...
export const DeleteKeys = ({ id }: { id: string }) => {
    ...
    return (
        <Fragment>
            {
                isLoading ? <Icons.spinner className="h-4 w-4 animate-spin text-red-500" /> : <Trash2 size={16} onClick={deleteKey} className=" text-red-500" />
            }
        </Fragment>
    )
}