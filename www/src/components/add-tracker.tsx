...
import { a11yDark as lightTheme } from 'react-syntax-highlighter/dist/esm/styles/prism'
import { Icons } from "./icons"
import { Button } from "./ui/button"
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { gruvboxLight as darkTheme } from "react-syntax-highlighter/dist/esm/styles/prism"
import { useTheme } from "next-themes";
import { useAtom } from "jotai";
import { trackerModalAtom } from "@/jotai/store";
import { siteConfig } from "@/config/site"
import { CheckIcon, Code2, X } from "lucide-react"
...
},
{
  ...
  description: "Download the plugin from github and install it manually in your wordpress site.",
  hideLangSelector: true
},
]


export const AddTracker = ({ websiteId, show }: { websiteId: string, show: boolean }) => {
  ...
  useEffect(() => {
    function changeLang() {
      const snippet = snippets.find((snippet) => snippet.name === selectedFramework)
      if (!snippet) {
        setSelected(snippets.find((snippet) => snippet.name === selectedFramework + `-${selectedLang}`)!)
      } else {
        setSelected(snippet)
      }
    }
      ...
      >
        ...
        <div className=" relative flex h-max flex-col rounded-md border  bg-gradient-to-tr from-gray-50 to-gray-200  px-4 pb-6 animate-in dark:from-black dark:to-gray-800/20">
          <div className=" flex items-center justify-between  py-2">
            ...
            <Button className=" ml-auto flex items-center" variant="ghost"
            ...
            <div className="group relative flex items-center">
              ...
              <span className="flex min-w-0 flex-col">
                <span className="text-sm font-semibold tracking-wide">
                  <SyntaxHighlighter language="bash" style={
                    theme === "dark" ? lightTheme : darkTheme
                  } wrapLines customStyle={{
                    background: "none",
                    fontSize: "0.8rem",
                    border: "none",
                  }}>
          ...
          <div className="group relative flex items-start gap-2">
            ...
            <div className=" bg-gradient-br relative flex h-max min-w-[600px] flex-col rounded-md  border from-gray-100 to-gray-300 pb-6 animate-in dark:from-black dark:to-gray-800/20 ">
              <div className=" flex  items-center justify-between space-y-2 border-b p-4 md:py-2">
                ...
                <div className=" flex w-max items-center gap-2">
                  <Select defaultValue="nextjs" onValueChange={(value) => setSelectedFramework(value)}>
                  ...
                  {
                    !selected.hideLangSelector && <Select defaultValue="tsx"
              ...
              <SyntaxHighlighter language={selected.lang ?? "jsx"} style={
                theme === "dark" ? lightTheme : darkTheme
              } wrapLines showLineNumbers customStyle={{
                background: "none",
                fontSize: "0.8rem",
                border: "none",
