...
const eventsApiSchema = rootApiSchema.merge(z.object({
    orderBy: z.record(z.enum(["createdAt", "updatedAt", "id"]), z.enum(['asc', 'desc'])).default({
        createdAt: "desc"
    }),
    where: z.object({
...
export const POST = async (req: Request) => {
    try {
        ...
        if (schema.success) {
            ...
            const events = await db.webEvent.findMany({
                where: {
