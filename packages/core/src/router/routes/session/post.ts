...
export const SessionPostSchema = RootApiTrackerSchema.merge(
  z.object({
    data: z.object({
      pathname: z.string(),
      referrer: z.string(),
  ...
  if (body.success) {
    ...
    if (ipAddress && !(await isLocalhost(ipAddress))) {
      const location = !options.disableLocation
        ? options.getLocation
          ? await options.getLocation(ipAddress)
          : await getLocation(ipAddress, req).catch(() => null)
        : { city: null, country: null };
      if (!location && !options.disableLocation)
