...
export async function main() {
  ...
  const program = new Command().name("loglib").description("Loglib CLI");
      ...
      if (adapter === "prisma") {
        ...
        if (existsSync("prisma/schema.prisma")) {
          ...
          if (installPrisma.install) {
            ...
            logger.warn(
              `You can install Prisma later by running${packageManager} ${
        ...
        logger.info(
          "You can now run `npx prisma migrate dev or npx prisma db push` to create or update your database schema"
        );
        ...
        if (installSupabase.install) {
          ...
          logger.warn(
            `Make sure to install supabase-js later by running${packageManager} ${
              packageManager === "npm" ? "install" : "add"
            }  @supabase/supabase-js`
          );
        }
        logger.info(
          "Important! Copy the SQL code from the following link: https://github.com/LogLib/loglib#next-js-with-supabase and paste it into your Supabase SQL editor to create the necessary tables for LogLib."
        );
      }
  ...
  program
    ...
    .description("Download maxmind database")
    .action(async () => {
      await download();
    });
  program
    .command("update")
    .description("Update Loglib")
    .action(async () => {
      const packageJSON = fs.readJSONSync("package.json") as PackageJson;
      const packages =
        packageJSON.dependencies &&
        Object.keys(packageJSON.dependencies)
          .filter((pkg) => pkg.startsWith("@loglib"))
          .map((pkg) => `${pkg}@latest`);
