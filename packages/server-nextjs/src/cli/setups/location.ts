    ...
    if (locationResolver.location === "no") {
        const locationResolver = await prompts({
            ...
            choices: [{
                ...
                selected: true,
                description: "If you use GeoLite2 database, you can't deploy your project on vercel or similar serverless platforms"
            }, {
                value: "disable",
                title: "Disable Location Resolver",
                description: "If disabled you won't be able to see your visitor's location"
            }, {
                value: "custom",
                ...
                tar.t().on('entry', (entry) => {
                    if (entry.path.endsWith('.mmdb')) {
                        ...
                        writeStream.on('finish', () => {
                            ...
                            installerSpinner.succeed("Maxmind GeoLite2 Database installed successfully")
                        });
                    }
                })
            );
