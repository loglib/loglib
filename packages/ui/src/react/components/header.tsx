...
export type HeaderProps = {
  ...
  logoutFn: () => void;
  timezones: Record<string, { name: string }>;
  hideLogout?: boolean;
};
  ...
  return (
    <div className="tw-flex tw-items-center tw-justify-between">
      <div className="tw-flex tw-items-center tw-space-x-2 ">
        <LogoIcon />
        <h2 className="md:tw-text-4xl tw-font-bold tw-tracking-tight tw-text-xl dark:tw-text-white/80">
          LOGLIB
        </h2>
      </div>
                  ...
                  <DropdownMenuSubContent className=" tw-h-52 md:tw-w-52 tw-w-44 tw-overflow-scroll">
                    {Object.keys(timezones)
                      ...
                      .map((t: string) => {
                        ...
                        return (
                          <DropdownMenuCheckboxItem
                            checked={z?.name === timezone}
                            onCheckedChange={() => z && setTimezone(z.name)}
                            key={t}
                          >
                            <span>{z?.name}</span>
                          </DropdownMenuCheckboxItem>
